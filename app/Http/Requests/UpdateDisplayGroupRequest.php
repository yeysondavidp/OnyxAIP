...
class UpdateDisplayGroupRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $groupId = $this->route('display_group')?->id;
        ...
        return [
            ...
            'player_asset_id' => [
                ...
                Rule::exists('assets', 'id')->where(fn ($q) => $q
                    ...
                    ->where(fn ($q2) => $q2
                        // allow the current group's player
                        ->whereNotIn('id', function ($q3) use ($groupId) {
                            $q3->select('player_asset_id')->from('display_groups')
