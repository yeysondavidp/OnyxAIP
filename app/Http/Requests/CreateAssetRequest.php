...
class CreateAssetRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $isInfra    = $type === AssetType::Infrastructure->value;
        $isFixture  = $type === AssetType::WindowFixture->value;

        $clientId = $this->input('client_id');

        return [
            // ── Base fields ────────────────────────────────────────────────
            'asset_code'      => ['required', 'string', 'max:40', 'unique:assets,asset_code'],
    ...
    public function messages(): array
    {
        return [
            ...
            'asset_type.enum'        => 'Please select a valid asset type.',
            'asset_status.enum'      => 'Please select a valid asset status.',
